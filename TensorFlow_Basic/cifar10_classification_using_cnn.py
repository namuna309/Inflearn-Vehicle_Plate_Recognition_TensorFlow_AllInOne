...
# CIFAR-10 데이터 다운로드 및 불러오기
(x_train, y_train), (x_test, y_test) = tf.keras.datasets.cifar10.load_data()
# 이미지들을 float32 데이터 타입으로 변경
x_train, x_test = x_train.astype('float32'), x_test.astype('float32')
# [0, 255] 사이의 값을 [0, 1] 사이의 값으로 Normalize
x_train, x_test = x_train / 255., x_test / 255.
# Scalar 형태의 레이블(0~9)을 one-hot Encoding 형태로 변환, sqeeze로 크기가 1인 차원 제거
y_train_one_hot = tf.squeeze(tf.one_hot(y_train, 10), axis=1)
y_test_one_hot = tf.squeeze(tf.one_hot(y_test, 10), axis=1)

# tf.data API를 통해 데이터를 섞고 batch 형태로 가져옴
...
class CNN(tf.keras.Model):
    def __init__(self):
        ...
        # Fully Connected Layer 2 - 384개의 특징들(feature)을 10개의 클래스-airplane, automobile, bird...-로 맵핑(mapsping)
...
for i in range(10000):
    ...
    if i % 100 == 0:
        ...
        loss_print = cross_entropy_loss(CNN_model(batch_x, is_training=False)[1], batch_y)

        print("반복(Epoch): %d, 트레이닝 데이터 정확도: %f, 손실 함수(loss): %f" % (i, train_accuracy, loss_print))
    # 20% 확률의 Dropout을 이용하여 학습 진행
...
for i in range(10):
    ...
    test_accuracy = test_accuracy + compute_accuracy(CNN_model(test_batch_x, is_training=False)[0], test_batch_y).numpy()
